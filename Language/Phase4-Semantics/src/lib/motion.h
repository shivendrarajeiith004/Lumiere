...
void calculate_displacement_final_vel(struct Body *body); // s = (v^2 - u^2) / (2a)
void calculate_displacement_avg_velocity(struct Body *body); // s = (u + v) * t / 2

// Additional functions for extended calculation
void calculate_time_from_displacement(struct Body *body); // t = (-u ± sqrt(u^2 + 2as)) / a
